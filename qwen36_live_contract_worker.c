...
static int fixture_load(const char *path, live_fixture *fx) {
    ...
    if (!read_exact(fp, &fx->layer, sizeof(uint32_t)) || !read_exact(fp, &fx->hidden, sizeof(uint32_t)) ||
        ...
        !read_exact(fp, &fx->topk, sizeof(uint32_t)) || !read_exact(fp, &fx->inter, sizeof(uint32_t))) { fclose(fp); return 0; }
...
static int run_prefill_layer_dynamic(const live_fixture *fx,
    ...
    for (t = 0; t < seq_len; ++t) {
        ...
        for (i = 0; i < fx->topk; ++i) {
            if (!ensure_expert_loaded(gf, layer, ls->experts, router_idx[i], fx->hidden, fx->inter, err, err_cap)) goto done;
            matvec(ls->experts[router_idx[i]].gate, post_row, gate, fx->inter, fx->hidden);
            matvec(ls->experts[router_idx[i]].up, post_row, up, fx->inter, fx->hidden);
...
static int run_step_layer_dynamic(const live_fixture *fx,
    ...
    gate = (float *)malloc((size_t)fx->inter * sizeof(float));
    up = (float *)malloc((size_t)fx->inter * sizeof(float));
    act = (float *)malloc((size_t)fx->inter * sizeof(float));
    down = (float *)malloc((size_t)fx->hidden * sizeof(float));
    shared_gate = (float *)malloc((size_t)fx->inter * sizeof(float));
    shared_up = (float *)malloc((size_t)fx->inter * sizeof(float));
    shared_act = (float *)malloc((size_t)fx->inter * sizeof(float));
...
static void worker_reset(worker_state *ws) {
    ...
    for (uint32_t li = 0; li < ws->n_fx; ++li) {
        const live_fixture *fx = &ws->fxs[li];
...
static void handle_prefill_seq_bin(worker_state *ws, const qwen36_gguf_file *gf, char *rest) {
    ...
    if (ws->fxs[0].layer == 0) {
        printf("ERROR PREFILL_SEQ_BIN requires non-blk.0 first live fixture\n");
        fflush(stdout);
        return;
    }
...
int main(int argc, char **argv) {
    ...
    if (argc < 3) {
        fprintf(stderr, "usage: %s MODEL.gguf LAYER0.live.bin [LAYER1.live.bin ...]\n", argv[0]);
        return 1;
    }
    ...
    ws.fxs = (live_fixture *)calloc(ws.n_fx, sizeof(live_fixture));
