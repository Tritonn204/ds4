...
typedef struct qwen36_35a3b_q4xl_expect {
    const char *name;
    uint32_t type;
    uint32_t ndim;
    uint64_t d0;
...
static bool tensor_matches(const qwen36_gguf_tensor *t, const qwen36_35a3b_q4xl_expect *e, char *err, size_t err_cap) {
    if (!t) {
...
static bool expect_named_tensor(const qwen36_gguf_file *gf, const qwen36_35a3b_q4xl_expect *e, char *err, size_t err_cap) {
...
static bool validate_metadata(const qwen36_gguf_file *gf, char *err, size_t err_cap) {
    ...
    if (!expect_kv_u64(gf, "general.file_type", 15, err, err_cap)) return false;
...
static bool bind_layer(const qwen36_gguf_file *gf, uint32_t i, qwen36_35a3b_q4xl_layer *l, char *err, size_t err_cap) {
    ...
    if (has_hybrid) {
        l->kind = QWEN36_Q4XL_LAYER_KIND_HYBRID_SSM;
        ...
        l->kind = QWEN36_Q4XL_LAYER_KIND_FULL_ATTENTION;
...
bool qwen36_35a3b_q4xl_validate(const qwen36_gguf_file *gf, char *err, size_t err_cap) {
    qwen36_35a3b_q4xl_model tmp;
    if (!validate_metadata(gf, err, err_cap)) return false;
    if (!validate_export_surface(gf, err, err_cap)) return false;
    if (!qwen36_35a3b_q4xl_bind(gf, &tmp, err, err_cap)) return false;
    return true;
}

bool qwen36_35a3b_q4xl_bind(const qwen36_gguf_file *gf, qwen36_35a3b_q4xl_model *out, char *err, size_t err_cap) {
    uint32_t i;
    qwen36_35a3b_q4xl_expect root_expect[] = {
    ...
    for (i = 0; i < QWEN36_35A3B_Q4XL_BLOCK_COUNT; i++) {
        if (!bind_layer(gf, i, &out->layers[i], err, err_cap)) return false;
        if (out->layers[i].kind == QWEN36_Q4XL_LAYER_KIND_HYBRID_SSM) out->hybrid_layer_count++;
        if (out->layers[i].kind == QWEN36_Q4XL_LAYER_KIND_FULL_ATTENTION) out->full_attn_layer_count++;
    }
...
void qwen36_35a3b_q4xl_dump_summary(const qwen36_35a3b_q4xl_model *m, FILE *fp) {
    uint32_t i;
    fprintf(fp, "target: Qwen3.6-35B-A3B UD_Q4_K_XL\n");
    fprintf(fp, "file: %s\n", m->gf->path);
    ...
    fputs("layer_kinds:\n", fp);
    for (i = 0; i < QWEN36_35A3B_Q4XL_BLOCK_COUNT; i++) {
        fprintf(fp, "  blk.%u: %s gate/up=%s down=%s\n", i,
                m->layers[i].kind == QWEN36_Q4XL_LAYER_KIND_HYBRID_SSM ? "hybrid_ssm" : "full_attention",
