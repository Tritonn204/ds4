...
typedef struct qwen36_35a3b_iq2xxs_expect {
    const char *name;
    uint32_t type;
    uint32_t ndim;
    uint64_t d0;
...
static bool tensor_matches(const qwen36_gguf_tensor *t, const qwen36_35a3b_iq2xxs_expect *e, char *err, size_t err_cap) {
    if (!t) {
...
static bool expect_named_tensor(const qwen36_gguf_file *gf, const qwen36_35a3b_iq2xxs_expect *e, char *err, size_t err_cap) {
...
static uint32_t expect_ffn_down_type(uint32_t layer) {
    ...
    case 38:
    case 39:
        return QWEN36_GGUF_TYPE_IQ3_S;
    default:
        return QWEN36_GGUF_TYPE_IQ2_S;
    }
}
...
static bool validate_metadata(const qwen36_gguf_file *gf, char *err, size_t err_cap) {
    ...
    if (!expect_kv_u64(gf, "general.file_type", 19, err, err_cap)) return false;
...
static bool validate_export_surface(const qwen36_gguf_file *gf, char *err, size_t err_cap) {
    ...
    for (i = 0; i < gf->tensor_count; i++) {
        ...
        if (type != QWEN36_GGUF_TYPE_F32 &&
            ...
            type != QWEN36_GGUF_TYPE_IQ2_S &&
            type != QWEN36_GGUF_TYPE_IQ3_S) {
...
static bool bind_layer(const qwen36_gguf_file *gf, uint32_t i, qwen36_35a3b_iq2xxs_layer *l, char *err, size_t err_cap) {
    ...
    if (!expect_layer_tensor(gf, i, "ffn_gate_exps.weight", QWEN36_GGUF_TYPE_IQ2_XXS, 3, 2048, 512, 256, &l->ffn_gate_exps, err, err_cap)) return false;
    if (!expect_layer_tensor(gf, i, "ffn_up_exps.weight", QWEN36_GGUF_TYPE_IQ2_XXS, 3, 2048, 512, 256, &l->ffn_up_exps, err, err_cap)) return false;
    if (!expect_layer_tensor(gf, i, "ffn_down_exps.weight", down_type, 3, 512, 2048, 256, &l->ffn_down_exps, err, err_cap)) return false;
    if (!expect_layer_tensor(gf, i, "ffn_gate_shexp.weight", QWEN36_GGUF_TYPE_Q5_K, 2, 2048, 512, 0, &l->ffn_gate_shexp, err, err_cap)) return false;
    if (!expect_layer_tensor(gf, i, "ffn_up_shexp.weight", QWEN36_GGUF_TYPE_Q5_K, 2, 2048, 512, 0, &l->ffn_up_shexp, err, err_cap)) return false;
    if (!expect_layer_tensor(gf, i, "ffn_down_shexp.weight", QWEN36_GGUF_TYPE_Q6_K, 2, 512, 2048, 0, &l->ffn_down_shexp, err, err_cap)) return false;
    ...
    if (has_hybrid) {
        l->kind = QWEN36_IQ2XXS_LAYER_KIND_HYBRID_SSM;
        if (!expect_layer_tensor(gf, i, "attn_gate.weight", QWEN36_GGUF_TYPE_Q5_K, 2, 2048, 4096, 0, &l->attn_gate, err, err_cap)) return false;
        if (!expect_layer_tensor(gf, i, "attn_qkv.weight", QWEN36_GGUF_TYPE_Q5_K, 2, 2048, 8192, 0, &l->attn_qkv, err, err_cap)) return false;
        if (!expect_layer_tensor(gf, i, "ssm_a", QWEN36_GGUF_TYPE_F32, 1, 32, 0, 0, &l->ssm_a, err, err_cap)) return false;
        if (!expect_layer_tensor(gf, i, "ssm_alpha.weight", QWEN36_GGUF_TYPE_F32, 2, 2048, 32, 0, &l->ssm_alpha, err, err_cap)) return false;
        if (!expect_layer_tensor(gf, i, "ssm_beta.weight", QWEN36_GGUF_TYPE_F32, 2, 2048, 32, 0, &l->ssm_beta, err, err_cap)) return false;
        ...
        if (!expect_layer_tensor(gf, i, "ssm_out.weight", QWEN36_GGUF_TYPE_Q6_K, 2, 4096, 2048, 0, &l->ssm_out, err, err_cap)) return false;
    } else {
        l->kind = QWEN36_IQ2XXS_LAYER_KIND_FULL_ATTENTION;
        if (!expect_layer_tensor(gf, i, "attn_q.weight", QWEN36_GGUF_TYPE_Q5_K, 2, 2048, 8192, 0, &l->attn_q, err, err_cap)) return false;
        if (!expect_layer_tensor(gf, i, "attn_q_norm.weight", QWEN36_GGUF_TYPE_F32, 1, 256, 0, 0, &l->attn_q_norm, err, err_cap)) return false;
        if (!expect_layer_tensor(gf, i, "attn_k.weight", QWEN36_GGUF_TYPE_Q5_K, 2, 2048, 512, 0, &l->attn_k, err, err_cap)) return false;
        if (!expect_layer_tensor(gf, i, "attn_k_norm.weight", QWEN36_GGUF_TYPE_F32, 1, 256, 0, 0, &l->attn_k_norm, err, err_cap)) return false;
        if (!expect_layer_tensor(gf, i, "attn_v.weight", QWEN36_GGUF_TYPE_Q5_K, 2, 2048, 512, 0, &l->attn_v, err, err_cap)) return false;
        if (!expect_layer_tensor(gf, i, "attn_output.weight", QWEN36_GGUF_TYPE_Q5_K, 2, 4096, 2048, 0, &l->attn_output, err, err_cap)) return false;
    }
    return true;
}

bool qwen36_35a3b_iq2xxs_validate(const qwen36_gguf_file *gf, char *err, size_t err_cap) {
    qwen36_35a3b_iq2xxs_model tmp;
    if (!validate_metadata(gf, err, err_cap)) return false;
    if (!validate_export_surface(gf, err, err_cap)) return false;
    if (!qwen36_35a3b_iq2xxs_bind(gf, &tmp, err, err_cap)) return false;
    return true;
}

bool qwen36_35a3b_iq2xxs_bind(const qwen36_gguf_file *gf, qwen36_35a3b_iq2xxs_model *out, char *err, size_t err_cap) {
    uint32_t i;
    qwen36_35a3b_iq2xxs_expect root_expect[] = {
        {"token_embd.weight", QWEN36_GGUF_TYPE_Q4_K, 2, 2048, 248320, 0},
        {"output_norm.weight", QWEN36_GGUF_TYPE_F32, 1, 2048, 0, 0},
        {"output.weight", QWEN36_GGUF_TYPE_Q4_K, 2, 2048, 248320, 0},
    };
    ...
    for (i = 0; i < QWEN36_35A3B_IQ2XXS_BLOCK_COUNT; i++) {
        if (!bind_layer(gf, i, &out->layers[i], err, err_cap)) return false;
        if (out->layers[i].kind == QWEN36_IQ2XXS_LAYER_KIND_HYBRID_SSM) out->hybrid_layer_count++;
        if (out->layers[i].kind == QWEN36_IQ2XXS_LAYER_KIND_FULL_ATTENTION) out->full_attn_layer_count++;
    }
...
void qwen36_35a3b_iq2xxs_dump_summary(const qwen36_35a3b_iq2xxs_model *m, FILE *fp) {
    uint32_t i;
    fprintf(fp, "target: Qwen3.6-35B-A3B UD_IQ2_XXS\n");
    fprintf(fp, "file: %s\n", m->gf->path);
    ...
    fputs("layer_kinds:\n", fp);
    for (i = 0; i < QWEN36_35A3B_IQ2XXS_BLOCK_COUNT; i++) {
        fprintf(fp, "  blk.%u: %s gate/up=%s down=%s attn=%s\n", i,
                m->layers[i].kind == QWEN36_IQ2XXS_LAYER_KIND_HYBRID_SSM ? "hybrid_ssm" : "full_attention",
                qwen36_gguf_type_name(m->layers[i].ffn_gate_exps->type),
                qwen36_gguf_type_name(m->layers[i].ffn_down_exps->type),
                m->layers[i].kind == QWEN36_IQ2XXS_LAYER_KIND_HYBRID_SSM
                    ? qwen36_gguf_type_name(m->layers[i].attn_qkv->type)
                    : qwen36_gguf_type_name(m->layers[i].attn_q->type));
    }
}
