...
typedef struct dec_fixture {
    ...
    float *post_attn_ln;
    float *mlp_out_ref;
    float *layer_output_ref;
    float *router_indices_f32;
    float *router_scores;
    float *gate_sel;
    float *up_sel;
    float *down_sel;
...
static void fixture_free(dec_fixture *fx) {
    ...
    free(fx->layer_output_ref); free(fx->router_indices_f32); free(fx->router_scores); free(fx->gate_sel);
    free(fx->up_sel); free(fx->down_sel); free(fx->gate_shexp); free(fx->up_shexp); free(fx->down_shexp);
    free(fx->gate_inp_shexp); free(fx->norm_w); free(fx->out_w);
...
#define R(name,count) if (!alloc_read_f32(fp, &fx->name, (count))) { fclose(fp); fixture_free(fx); return 0; }
    R(layer_input, fx->hidden);
    R(q, (size_t)fx->seq_len * fx->num_v_heads * fx->head_k_dim);
    R(k, (size_t)fx->seq_len * fx->num_v_heads * fx->head_k_dim);
    R(v, (size_t)fx->seq_len * fx->num_v_heads * fx->head_v_dim);
    R(beta, (size_t)fx->seq_len * fx->num_v_heads);
    R(g, (size_t)fx->seq_len * fx->num_v_heads);
    R(z_last, (size_t)fx->num_v_heads * fx->head_v_dim);
    R(mixer_out_ref, fx->hidden);
    R(residual_after_mixer_ref, fx->hidden);
    R(post_attn_ln, fx->hidden);
    R(mlp_out_ref, fx->hidden);
    R(layer_output_ref, fx->hidden);
    R(router_indices_f32, fx->topk);
    R(router_scores, fx->topk);
    R(gate_sel, (size_t)fx->topk * fx->inter * fx->hidden);
    R(up_sel, (size_t)fx->topk * fx->inter * fx->hidden);
    R(down_sel, (size_t)fx->topk * fx->hidden * fx->inter);
    ...
    R(norm_w, fx->head_v_dim);
    R(out_w, (size_t)fx->hidden * fx->num_v_heads * fx->head_v_dim);
#undef R
    fclose(fp);
    return 1;
}
...
int main(int argc, char **argv) {
    dec_fixture fx;
    float *state = NULL, *core = NULL, *out_in = NULL, *mixer = NULL, *resid = NULL, *mlp = NULL, *final_out = NULL;
    ...
    out_in = (float *)calloc((size_t)fx.num_v_heads * fx.head_v_dim, sizeof(float));
    ...
    final_out = (float *)calloc(fx.hidden, sizeof(float));
    gate = (float *)calloc(fx.inter, sizeof(float));
    up = (float *)calloc(fx.inter, sizeof(float));
    act = (float *)calloc(fx.inter, sizeof(float));
    down = (float *)calloc(fx.hidden, sizeof(float));
    shared_gate = (float *)calloc(fx.inter, sizeof(float));
    shared_up = (float *)calloc(fx.inter, sizeof(float));
    shared_act = (float *)calloc(fx.inter, sizeof(float));
    shared = (float *)calloc(fx.hidden, sizeof(float));
    if (!state || !core || !out_in || !mixer || !resid || !mlp || !final_out || !gate || !up || !act || !down || !shared_gate || !shared_up || !shared_act || !shared) {
        fixture_free(&fx);
        return 1;
    }
    ...
    for (h = 0; h < fx.num_v_heads; ++h) {
        ...
        for (vd = 0; vd < fx.head_v_dim; ++vd) {
            double cv = core[((size_t)(fx.seq_len - 1) * fx.num_v_heads + h) * fx.head_v_dim + vd];
            var += cv * cv;
        }
    ...
    for (i = 0; i < fx.hidden; ++i) resid[i] = fx.layer_input[i] + mixer[i];

    for (i = 0; i < fx.topk; ++i) {
        const float *gate_mat = fx.gate_sel + (size_t)i * fx.inter * fx.hidden;
        const float *up_mat = fx.up_sel + (size_t)i * fx.inter * fx.hidden;
        const float *down_mat = fx.down_sel + (size_t)i * fx.hidden * fx.inter;
        matvec(gate_mat, fx.post_attn_ln, gate, fx.inter, fx.hidden);
        matvec(up_mat, fx.post_attn_ln, up, fx.inter, fx.hidden);
        for (vd = 0; vd < fx.inter; ++vd) act[vd] = siluf_local(gate[vd]) * up[vd];
        matvec(down_mat, act, down, fx.hidden, fx.inter);
        for (vd = 0; vd < fx.hidden; ++vd) mlp[vd] += down[vd] * fx.router_scores[i];
    }
    matvec(fx.gate_shexp, fx.post_attn_ln, shared_gate, fx.inter, fx.hidden);
    matvec(fx.up_shexp, fx.post_attn_ln, shared_up, fx.inter, fx.hidden);
    ...
    {
        float scale_in = 0.0f;
        for (vd = 0; vd < fx.hidden; ++vd) scale_in += fx.post_attn_ln[vd] * fx.gate_inp_shexp[vd];
        {
    ...
    for (vd = 0; vd < fx.hidden; ++vd) final_out[vd] = resid[vd] + mlp[vd];
    ...
    fixture_free(&fx);
    free(state); free(core); free(out_in); free(mixer); free(resid); free(mlp); free(final_out);
