...
static int run_layer_dynamic(const dwf_fixture *fx, const qwen36_gguf_file *gf, const qwen36_35a3b_q8_layer *layer, expert_cache_entry *cache,
    ...
    for (t = 0; t < seq_len; ++t) {
        ...
        for (i = 0; i < fx->topk; ++i) {
            if (!ensure_expert_loaded(gf, layer, cache, router_idx[i], fx->hidden, err, err_cap)) goto oom;
            matvec(cache[router_idx[i]].gate, post_row, gate, INTER, fx->hidden);
            matvec(cache[router_idx[i]].up, post_row, up, INTER, fx->hidden);
            for (vd = 0; vd < INTER; ++vd) act[vd] = siluf_local(gate[vd]) * up[vd];
            matvec(cache[router_idx[i]].down, act, down, fx->hidden, INTER);
...
int main(int argc, char **argv) {
    ...
    if (argc < 4) { fprintf(stderr, "usage: %s MODEL.gguf PREFIX.bin LAYER0.bin [LAYER1.bin ...] [--use-prefix-input-seq] [--dump-last PATH] [--dump-seq PATH]\n", argv[0]); return 1; }
    gguf_path = argv[1];
    prefix_path = argv[2];
    ...
    for (int i = 0; i < n_fx; ++i) {
        ...
        printf("run_layer_dynamic: blk.%u seq_len=%u\n", fxs[i].layer, pfx.seq_len);
        if (!run_layer_dynamic(&fxs[i], &gf, &q8.layers[fxs[i].layer], caches + (size_t)i * ROUTER_COUNT, pfx.seq_len, state_seq, next_seq, err, sizeof(err))) {
            fprintf(stderr, "run_layer_dynamic failed: blk.%u: %s\n", fxs[i].layer, err);
            free(next_seq);
            goto fail;
        }
