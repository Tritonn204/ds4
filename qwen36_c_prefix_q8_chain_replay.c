...
static int decode_q8_row(const qwen36_gguf_file *gf, const qwen36_gguf_tensor *t, uint32_t row_idx, uint32_t hidden, float *out, char *err, size_t err_cap) {
    const size_t row_size = 34u * (hidden / 32u);
    uint8_t *buf = (uint8_t *)malloc(row_size);
    if (!buf) return 0;
    if (!qwen36_gguf_read_tensor_bytes(gf, t, (uint64_t)row_idx * row_size, buf, row_size, err, err_cap)) { free(buf); return 0; }
    for (uint32_t j = 0; j < hidden / 32u; ++j) {
        const uint8_t *block = buf + j * 34u;
        const float dscale = f16_to_f32((uint16_t)(block[0] | (block[1] << 8)));
        const int8_t *qs = (const int8_t *)(block + 2);
        for (uint32_t k = 0; k < 32u; ++k) out[j * 32u + k] = dscale * (float)qs[k];
    }
    free(buf);
    return 1;
}
...
static int run_layer(const dwf_fixture *fx, const float *input_override_seq, float *out_final_seq, layer_debug *dbg) {
    ...
    for (t = 0; t < fx->seq_len; ++t) {
        ...
        var /= (double)fx->hidden;
        for (d = 0; d < fx->hidden; ++d) { post_row[d] = (float)(resid_row[d] / sqrt(var + 1e-6)) * fx->post_attn_norm_w[d]; scale_in += post_row[d] * fx->gate_inp_shexp[d]; }
        for (i = 0; i < fx->topk; ++i) {
    ...
    if (dbg) {
        dbg->input_ln_seq = input_ln; dbg->qkv_seq = qkv; dbg->z_seq = z; dbg->a_seq = a; dbg->b_seq = b;
        dbg->out_proj_seq = out_proj; dbg->resid_seq = resid_seq; dbg->post_attn_ln_seq = post_attn_ln_seq; dbg->mlp_seq = mlp_seq;
    } else {
...
int main(int argc, char **argv) {
    ...
    char err[512];
    if (argc < 4) { fprintf(stderr, "usage: %s MODEL.gguf PREFIX.bin LAYER0.bin [LAYER1.bin ...] [--dump-last PATH] [--dump-seq PATH]\n", argv[0]); return 1; }
    gguf_path = argv[1];
    prefix_path = argv[2];
    ...
    if (!fxs) { qwen36_gguf_close(&gf); prefix_free(&pfx); return 1; }
    {
        int fxi = 0;
        for (argi = 3; argi < argc; ++argi) {
            if (strcmp(argv[argi], "--dump-last") == 0 || strcmp(argv[argi], "--dump-seq") == 0) { ++argi; continue; }
            if (!fixture_load(argv[argi], &fxs[fxi])) { fprintf(stderr, "failed to load fixture: %s\n", argv[argi]); goto fail; }
            if (fxs[fxi].seq_len != pfx.seq_len) {
                fprintf(stderr, "fixture seq_len mismatch: prefix=%u fixture[%d]=%u path=%s\n", pfx.seq_len, fxi, fxs[fxi].seq_len, argv[argi]);
                goto fail;
            }
            if (fxs[fxi].hidden != pfx.hidden) {
                fprintf(stderr, "fixture hidden mismatch: prefix=%u fixture[%d]=%u path=%s\n", pfx.hidden, fxi, fxs[fxi].hidden, argv[argi]);
                goto fail;
            }
            ++fxi;
        }
    ...
    printf("embedding_rmse: %.8f\n", vec_rmse(input_seq, pfx.input_seq_ref, (size_t)pfx.seq_len * pfx.hidden));
    printf("embedding_cosine: %.8f\n", vec_cosine(input_seq, pfx.input_seq_ref, (size_t)pfx.seq_len * pfx.hidden));
    ...
    for (int i = 0; i < n_fx; ++i) {
        ...
        if (!run_layer(&fxs[i], state_seq, next_seq, &dbg)) { free(next_seq); goto fail; }
    ...
    free(fxs); if (state_seq != input_seq) free(state_seq); free(input_seq); prefix_free(&pfx); qwen36_gguf_close(&gf);
    return 0;
fail:
    for (int i = 0; i < n_fx; ++i) fixture_free(&fxs[i]);
    free(fxs); if (state_seq && state_seq != input_seq) free(state_seq); free(input_seq); prefix_free(&pfx); qwen36_gguf_close(&gf);
    return 1;
}
