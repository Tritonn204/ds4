...
def main() -> int:
    ap = argparse.ArgumentParser(description="Export full linear-attention layer fixture for C replay")
    ...
    q = np.asarray(bundle["query"], dtype=np.float32)
    k = np.asarray(bundle["key"], dtype=np.float32)
    v = np.asarray(bundle["value"], dtype=np.float32)
    beta = np.asarray(bundle["beta"], dtype=np.float32)
    g = np.asarray(bundle["g"], dtype=np.float32)
    ...
    with out_path.open("wb") as fp:
        fp.write(MAGIC)
        fp.write(struct.pack("<IIIIIIII", layer, seq_len, 32, 16, 128, 128, 4096, 2048))
        for arr in (layer_input, q, k, v, beta, g, z_last, mixer_out, residual_after_mixer, post_attn_ln, layer_out, norm_w, out_w):
